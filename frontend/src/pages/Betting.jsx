...
export default function Betting() {
  ...
  function validateMultipliers() {
    if (multipliers.some(value => value.trim() === '' || !Number.isFinite(Number(value)) || Number(value) < 0)) {
      setError('Enter a custom multiplier of zero or more for every cup.');
      return false;
    }
    setError('');
    return true;
  ...
  return <><h1>Auction cup betting</h1><p>Bets are deducted immediately. Results return the stake multiplied by the outcome. Auction payouts are rounded down to whole points.</p>
    <section aria-label="Betting instructions"><p>1. Select a betting game. 2. Enter the team ID and check its balance. 3. Place the bet. 4. Reveal the results below.</p><p><strong>Auction multipliers:</strong> enter any nonnegative value for each cup, such as 0, 1.25 or 3. Cups may use the same multiplier.</p></section>
    ...
    <fieldset className="portal-fields" disabled={busy}>
      ...
      {!team?.activeBet && !completed && <form className="portal-form" onSubmit={event => { event.preventDefault(); act(placeBet); }}><fieldset className="portal-fields" disabled={!team || !game}><label>Points to bet<input required type="number" min="1" max={team?.totalPoints} step="1" value={amount} onChange={event => setAmount(event.target.value)} /></label><label>Cup<select value={selection} onChange={event => setSelection(event.target.value)}>{Array.from({ length: 4 }, (_, i) => <option key={i} value={i + 1}>{i + 1}</option>)}</select></label><p>{team ? `After placing this bet: ${team.totalPoints - Number(amount || 0)} points` : 'Check a team to see its available balance.'}</p><button className="portal-button">Place bet and deduct points</button></fieldset>{(!team || !game) && <p>Select a betting game and check the team ID to enable betting.</p>}</form>}
      {game && <><h2>Active bets</h2><div className="portal-table-wrap"><table className="portal-table"><thead><tr><th>Team</th><th>Selection</th><th>Bet</th></tr></thead><tbody>{bets.map(bet => <tr key={bet.teamId}><td>{bet.teamName} · #{bet.teamId}</td><td>{bet.cupNumber}</td><td>{bet.pointsBet}</td></tr>)}</tbody></table></div>{!bets.length && <p>No active bets.</p>}
        {completed ? <p>This round is complete. Create a new game for the next round.</p> : <><h2>Reveal cup multipliers</h2><p>Enter a custom multiplier for each cup. Use 0 for no payout or 1 to return the stake.</p><div className="portal-form">{multipliers.map((value, i) => <label key={i}>Cup {i + 1} multiplier<input type="number" min="0" step="any" required placeholder="e.g. 2.5" value={value} onChange={event => { setConfirm(false); setMultipliers(old => old.map((v, j) => j === i ? event.target.value : v)); }} /></label>)}</div><button className="portal-button" disabled={!bets.length} onClick={() => { if (validateMultipliers()) setConfirm(true); }}>Review results</button>{confirm && <section className="portal-success"><p>Finalize these results for {game.gameName} and update {bets.length} team balances? This completes the round.</p><button className="portal-button" onClick={() => act(settle)}>Confirm results and update points</button> <button className="portal-button secondary" onClick={() => setConfirm(false)}>Cancel</button></section>}</>}
      </>}
    </fieldset>
