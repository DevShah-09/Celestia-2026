...
export default function AdminLayout() {
  ...
  return <PageShell><section className="portal-panel">
    {expired ? <><h1>Session expired</h1><p>Please sign in again to continue.</p><button className="portal-button" onClick={() => saveAdmin(null)}>Return to login</button></> : !profile ? <>
      ...
      <nav className="admin-tabs" aria-label="Organizer navigation">{[['/admin', 'Overview'], ['/admin/games', 'Games'], ['/admin/betting', 'Betting'], ['/admin/register', 'Register team'], ['/admin/scoring', 'Scoring / QR'], ['/admin/bulkupdate', 'Bulk scoring'], ['/admin/teams', 'Teams'], ['/admin/history', 'History'], ['/admin/admins', 'Admins']].map(([to, title]) => <NavLink key={to} to={to} end>{title}</NavLink>)}</nav>
