...
test('auction deducts stakes, pays all four multipliers, persists fields and prevents repeat settlement', async t => {
  ...
  const res = await call(revealAuctionResults, { gameId: game.id, cupMultipliers: { 1: 0, 2: 0.5, 3: 1.5, 4: 2 } });
  ...
  assert.equal((await call(revealAuctionResults, { gameId: game.id, cupMultipliers: { 1: 0, 2: 0.5, 3: 1.5, 4: 2 } })).statusCode, 400);
  assert.deepEqual(teams.map(team => team.totalPoints), [900, 950, 1050, 1100]);
});
...
test('invalid cup mappings and negative multipliers are rejected; fractional payouts round down', async t => {
  const { game, teams } = setup(t, 'auction');
  await call(placeAuctionBet, { gameId: game.id, teamId: 1, cupNumber: 2, pointsBet: 101 });
  for (const cupMultipliers of [{ a: 0, b: 0.5, c: 1.5, d: 2 }, { 1: 0, 2: -1, 3: 1.5, 4: 2 }]) assert.equal((await call(revealAuctionResults, { gameId: game.id, cupMultipliers })).statusCode, 400);
  assert.equal(teams[0].totalPoints, 899);
  await call(revealAuctionResults, { gameId: game.id, cupMultipliers: { 1: 0, 2: 0.5, 3: 1.5, 4: 2 } });
...
test('custom and repeated multipliers pay correctly, including profits below 1.5x', async t => {
  ...
  const res = await call(revealAuctionResults, { gameId: game.id, cupMultipliers: { 1: 1.25, 2: 3.75, 3: 3.75, 4: 0 } });
  assert.equal(res.statusCode, 200);
...
test('invalid or overflowing custom multipliers leave all balances unchanged', async t => {
  ...
  for (const value of [null, '', '2', -1, Infinity, Number.MAX_VALUE]) {
    const res = await call(revealAuctionResults, { gameId: game.id, cupMultipliers: { 1: value, 2: 1, 3: 1, 4: 1 } });
